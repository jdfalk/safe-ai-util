...
# Build cross-platform binaries for copilot-agent-util (legacy name) and safe-ai-util (new name)

set -e

echo "Building cross-platform binaries for copilot-agent-util and safe-ai-util..."
...
cp target/aarch64-apple-darwin/release/safe-ai-util dist/safe-ai-util-macos-arm64

# Build for macOS Intel
echo "Building for macOS x86_64..."
cargo build --release --target x86_64-apple-darwin
cp target/x86_64-apple-darwin/release/copilot-agent-util dist/copilot-agent-util-macos-x86_64
cp target/x86_64-apple-darwin/release/safe-ai-util dist/safe-ai-util-macos-x86_64
...
cp target/x86_64-unknown-linux-musl/release/safe-ai-util dist/safe-ai-util-linux-x86_64
...
if rustup target list --installed | grep -q "aarch64-unknown-linux-musl"; then
    ...
    cp target/aarch64-unknown-linux-musl/release/safe-ai-util dist/safe-ai-util-linux-arm64
else
