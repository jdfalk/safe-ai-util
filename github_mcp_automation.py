...
def categorize_issue(title: str, labels: List[str], body: str = "") -> str:
    ...
    for category, config in PROJECT_CATEGORIES.items():
        ...
        score += (
            sum(1 for keyword in config["title_keywords"] if keyword in title_lower) * 2
        )
...
def create_assignment_summary(repo_info: Dict, categorized_issues: Dict):
    ...
    for category, issues in categorized_issues.items():
        ...
        assignment_data["categories"][category] = {"count": len(issues), "issues": []}
    ...
    assignment_file = (
        f"/Users/jdfalk/repos/github.com/jdfalk/{repo}/project_assignments_{repo}.json"
    )
