...
def get_comprehensive_labels(
    title: str, existing_labels: List[str] = None
) -> List[str]:
...
def generate_labeling_commands() -> List[str]:
    ...
    for issue in gcommon_issues:
        comprehensive_labels = get_comprehensive_labels(
            issue["title"], issue["current_labels"]
        )
        commands.append(
            f"# Apply comprehensive labeling to gcommon issue #{issue['number']}"
        )
    ...
    for issue in ghcommon_issues:
        ...
        commands.append(
            f"# Apply comprehensive labeling to ghcommon issue #{issue['number']}"
        )
        commands.append(f"mcp_github_update_issue(")
        commands.append(f'    owner="jdfalk",')
    ...
    for issue in rust_issues:
        ...
        commands.append(
            f"# Apply comprehensive labeling to copilot-agent-util-rust issue #{issue['number']}"
        )
        commands.append(f"mcp_github_update_issue(")
        commands.append(f'    owner="jdfalk",')
...
def main():
    ...
    print(
        f"Generated {len([c for c in commands if c.startswith('mcp_')])} labeling commands"
    )
